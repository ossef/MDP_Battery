...
max_iter = 1e5
MDP_Case = 6   #Choose one of the models : The four last models are generated with "XBorne Tool, ISCIS 2016"
               ...
               #4 : Robertazzi     :  large Ngreen Models, a model for optical container filling, details of the model in papaer "YHJM18" published in "WIMOB 2018"
...
#---------> RoberTazzi type "B" NGreen optical model, "Sparse, Ngreen, Xborne" : data from "./Models/Ngreen/" directory
elif MDP_Case == 4:
    #Generated with XBorne tool, stored in "./Models/Ngreen/" directory

    index_model = 4 #choose one of the models
    models = [50, 500, 1000, 2000, 5000, 10000, 20000, 30000, 50000, 100000]
    sizes  = [52, 501, 1006, 2010, 5050, 10028, 20078, 30009, 50001, 100015]

    model= '../Models/Ngreen//Model_N_'+str(models[index_model])+'/'
    ...
    def generate_MDP() :
        ...
        myGraph = Graph(model,0)  #Initalize and read a ".sz" file, "0" pour le modéle sans phase
        N = myGraph.N 

        #--------- read TPM(a) : Matrixes model from external file for --------
        All_Ps = []
        for a in range(A) :
            myGraph.read_Rii_Matrixe(a,0)
...
#---------> RoberTazzi type "B" approx Battery model, one phase !, "Sparse, Battery, Xborne" : data from "./Models/Batterie/One_Phase" directory
elif MDP_Case == 5:
    #Generated with XBorne tool, stored in "./Models/Batterie/One_Phase" directory
    ...
    model= '../Models/Battery/One_Phase/Model_B_'+str(Buffer[index_model])+'_N_'+str(models[index_model])+'/'
...
#---------> RoberTazzi type "B" model, Battery model, two phases !, "Sparse, Battery, Xborne" : data from "./Models/Batterie3/Two_Phases_Scaling/" directory
elif MDP_Case == 6:
    #Generated with XBorne tool, stored in ./Models/Batterie3/Two_Phases_Scaling/" directory
    ...
    model= '../Models/Battery/Two_Phases_Scaling/Model_B_'+str(Buffer[index_model])+'_N_'+str(models[index_model])+'/'
    ...
    def generate_MDP() :
        ...
        myGraph = Graph(model,1)  #Initalize and read a ".sz" file, "1" pour le modéle with two phases
        N = myGraph.N 

        #--------- read TPM(a) : Matrixes model from external file for --------
        All_Ps = []
        for a in range(A) :
            myGraph.read_Rii_Matrixe(a,1)
            Ps = myGraph.csr_sparse
            All_Ps.append(Ps)
        All_Ps = np.array(All_Ps)
...
#---------> RoberTazzi type "B" model, Battery model, two phases !, "Sparse, Battery, Xborne" : data from "./Models/Batterie3/Two_Phases_Scenarios/" directory
elif MDP_Case == 7:
    #Generated with XBorne tool, stored in ./Models/Batterie3/Two_Phases_Scenarios/" directory
    ...
    model= '../Models/Battery/Two_Phases_Scenarios/Model_B_'+str(Buffer[index_model])+'_N_'+str(models[index_model])+'/'
    ...
    def generate_MDP(All_Ps, states, r1, r2, r3, r4, read) :
        if(read == 0):
            ...
            myGraph = Graph(model,1)  #Initalize and read a ".sz" file, '1' for model with two phases
            N = myGraph.N 
