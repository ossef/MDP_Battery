...
{ 
	char city[100], path[200];
  int fine;

if(argc !=4 )
 {
    printf("usage : sparsegthLD -f filename year \n");
    ...
    exit(1); 
      ...
      printf(" ******************************************************************\n\n");
    	...
    	strcpy(city,argv[3]);
	    snprintf(path, sizeof(path), "%s/%s", city, argv[2]);

      Construire_Matrice_Transition(path);
...
/* -- Ecriture de la distribution stationnaire dans un fichier */
	if((fic_resultat= (char *) calloc ( (150) , sizeof(char)))==NULL) {printf("pas de mem \n");exit(1);}
	strcpy(fic_resultat,path);
