...
static int Min[NEt], Max[NEt], Buffer, idAction;
static int seuil;
int  NbEvtsPossibles; 
...
void CreerMark()
{
	 ...
	 */
	if (!(Mark=(int *)calloc(MaxSize, sizeof(int)))) ProblemeMemoire();
...
void  usage()
{
	printf("usage : GenerMatrix -f filename seuil BufferSize idAction city \n");
...
char *argv[];
{
	...
	item It, It2;
	int j, k, bcli;
	long double SommeDeProba;
	int deja;
	long double pro, pro1, pro2;
	FILE *pf1, *pf2;
	char s [100];
	char s1[100];
	char s2[100];
	...
	switch (*++*argv) {
		...
		{++argv;
			...
			//Deadline =atoi (*(++argv));
			Buffer = atoi (*(++argv));
			idAction = atoi (*(++argv)) - 1;
			strcpy(city,*(++argv));
			snprintf(path, sizeof(path), "%s/%s", city, s);

			/* on ajoute .sz, .cd ou .Rii */
			strcpy(s1,path);  // "Fairbanks/Fairbanks_a1"
			strcpy(s2,path);
			strcpy(s3,path);
			strcat(s1,".cd"); // "Fairbanks/Fairbanks_a1.cd"
			strcat(s2,".Rii");
			strcat(s3,".sz");
			...
			printf("---> %s : Thr = %.2f%%, Buffer = %d, Action = %d \n",s,((float)seuil/Buffer)*100,Buffer,idAction);
			pf1=fopen(s1,"w");
			pf2=fopen(s2,"w"); 
			break;
		}
	...
	InitPoubelle();
	NbEvtsPossibles = ReadDistribs(Buffer, idAction, city, s);
	printf("NbEvtsPossibles = %d \n", NbEvtsPossibles);
	CreerMark();
	InitMark();
	NbArcs = 0;
	NbSommets = 0;
	...
	do {
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(It.voisin);
#endif
			Equation(It.voisin, j, F, R,seuil);
#if (DEBUG==1)
			PrintEtat(F);
#endif
    ...
    for (j = 1; j <= bcli; j++) {
		 ...
		 */
		fprintf(pf2,"% .25LE%12d", It2.prob, It2.numero);
	...
	{
		printf("attention la somme vaut : % .10LE\n", SommeDeProba);
		exit(0);
	}
