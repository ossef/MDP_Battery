...
int a[NbEvtsPossibles] = {-1,1,2};     //le service (-1), le support des arrivées (1 et 2)
...
double seuil;
{
  ...
  if(F[0] == 0 && F[1] == 0 && (a[indexevt-1] == 0 || indexevt == 1) ) //boucler pour un service en (0,0), ou une arrivée nulle
	{   bool = 1; }
  if( (F[0]>=seuil*Max[0]) || (F[1] == Max[1]) )         //Remplissage d'une nouvelle batterie
	{   F[0] =0; F[1] =0; bool=1; }
  if(bool==0 && F[0]+a[indexevt-1]>=0 && F[1]<Max[1] )   //Arrivée d'energie (dont service pour la première arrivée)
   { 
	  F[0] = min(F[0]+a[indexevt-1], Max[0]);              //Pas de débordement, pertes de paquets
	  F[1]++;
   }
}
