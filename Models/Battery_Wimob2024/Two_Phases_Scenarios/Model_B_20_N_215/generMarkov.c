...
static int Min[NEt], Max[NEt], Deadline, Buffer, idAction;
static double seuil;
...
void  usage()
{
	printf("usage : GenerMatrix -f filename seuil deadline BufferSize idAction \n");
...
char *argv[];
{
	...
	switch (*++*argv) {
		...
		{++argv;
			...
			printf("seuil = %.2f%%,  deadline = %d, Buffer = %d, action = %d \n",(seuil/Buffer)*100,Deadline,Buffer,idAction);
			pf1=fopen(s1,"w");
			pf2=fopen(s2,"w"); 
			break;
		}
	...
	InitPoubelle();
	InitEtendue(Deadline,Buffer,idAction);
	InitParticuliere();
	CreerMark();
	InitMark();
	NbArcs = 0;
	...
	do {
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(It.voisin);
#endif
			Equation(It.voisin, j, F, R,seuil,Deadline);
#if (DEBUG==1)
			PrintEtat(F);
#endif
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			for (k = 1; k <= NbEvtsPossibles; k++) {
				Equation(F, k, G, R,seuil,Deadline);
#if (DEBUG==2)
				PrintEtat(G);
#endif
